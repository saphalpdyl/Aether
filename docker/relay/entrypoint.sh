...
# Prefer explicit interface selection from env generated by topology.
uplink="${RELAY_UPLINK_IFACE:-}"
if [ -z "$uplink" ]; then
  uplink=$(echo "$ifaces" | tail -n 1)
fi

if ! echo "$ifaces" | grep -qx "$uplink"; then
  echo "Configured RELAY_UPLINK_IFACE '$uplink' not found in data-plane ifaces: $(echo "$ifaces" | tr '\n' ' ')"
  exec sleep infinity
fi

if [ -n "${RELAY_ACCESS_IFACES:-}" ]; then
  access_ifaces=$(echo "$RELAY_ACCESS_IFACES" | tr ',' ' ')
else
  access_ifaces=$(echo "$ifaces" | grep -vx "$uplink" || true)
fi

if [ -z "$access_ifaces" ]; then
  echo "No access interfaces resolved (uplink=$uplink); refusing to start relay"
  exec sleep infinity
fi
